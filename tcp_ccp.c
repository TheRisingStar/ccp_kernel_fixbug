...
void tcp_ccp_in_ack_event(struct sock *sk, u32 flags) {
    ...
    struct ccp *ca = inet_csk_ca(sk);//return the date struct of CCP CC
    struct ccp_primitives *mmt;//original date get from ccp
    u32 acked_bytes;
#ifdef COMPAT_MODE
    int i=0;
...
//zjh modified!
   // printk("ack_us=%lld,snd_us=%lld\n",ack_us,snd_us);
    if (ack_us != 0 && snd_us != 0) {
        rin = rout = (u64)rs->delivered * MTU * S_TO_US;
//zjh modified!
   // printk("rin=%lld,rout=%lld\n",rin,rout);
        do_div(rin, snd_us);
        do_div(rout, ack_us);
    }
//zjh modified!
   // printk("after do_div(),rin=%lld,rout=%lld\n",rin,rout);
...
struct ccp_connection *conn;
	...
	//folowing 4 line code added by zjh!
